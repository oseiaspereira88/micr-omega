...
let MIN_PLAYERS_TO_START = runtimeConfig.MIN_PLAYERS_TO_START;
let WAITING_START_DELAY_MS = runtimeConfig.WAITING_START_DELAY_MS;
let WAITING_START_DELAY_ENABLED = MIN_PLAYERS_TO_START > 1;
let ROUND_DURATION_MS = runtimeConfig.ROUND_DURATION_MS;
let RESET_DELAY_MS = runtimeConfig.RESET_DELAY_MS;
let RECONNECT_WINDOW_MS = runtimeConfig.RECONNECT_WINDOW_MS;
let INACTIVE_TIMEOUT_MS = runtimeConfig.INACTIVE_TIMEOUT_MS;
export let MAX_PLAYERS = runtimeConfig.MAX_PLAYERS;
export let MAX_CLIENT_MESSAGE_SIZE_BYTES = runtimeConfig.MAX_CLIENT_MESSAGE_SIZE_BYTES;

let MAX_COMBO_MULTIPLIER = runtimeConfig.MAX_COMBO_MULTIPLIER;
let DAMAGE_POPUP_TTL_MS = runtimeConfig.DAMAGE_POPUP_TTL_MS;
let MAX_DAMAGE_POPUPS_PER_TICK = runtimeConfig.MAX_DAMAGE_POPUPS_PER_TICK;

// Allow clients to sustain at least 70 messages per second without being
// throttled by using a one-minute sliding window.
export let RATE_LIMIT_WINDOW_MS = runtimeConfig.RATE_LIMIT_WINDOW_MS;
export let MAX_MESSAGES_PER_CONNECTION = runtimeConfig.MAX_MESSAGES_PER_CONNECTION;
// Base global rate limit; the runtime cap scales with the number of active sockets.
export let MAX_MESSAGES_GLOBAL = runtimeConfig.MAX_MESSAGES_GLOBAL;
export let GLOBAL_RATE_LIMIT_HEADROOM = runtimeConfig.GLOBAL_RATE_LIMIT_HEADROOM;
let RATE_LIMIT_UTILIZATION_REPORT_INTERVAL_MS =
  runtimeConfig.RATE_LIMIT_UTILIZATION_REPORT_INTERVAL_MS;
...
export let WORLD_TICK_INTERVAL_MS = runtimeConfig.WORLD_TICK_INTERVAL_MS;
let SNAPSHOT_FLUSH_INTERVAL_MS = runtimeConfig.SNAPSHOT_FLUSH_INTERVAL_MS;
let PLAYER_ATTACK_COOLDOWN_MS = runtimeConfig.PLAYER_ATTACK_COOLDOWN_MS;
let PLAYER_COLLECT_RADIUS = runtimeConfig.PLAYER_COLLECT_RADIUS;
export let CONTACT_BUFFER = runtimeConfig.CONTACT_BUFFER;
let PLAYER_ATTACK_RANGE_BUFFER = CONTACT_BUFFER;
export let PLAYER_COLLISION_RADIUS = runtimeConfig.PLAYER_COLLISION_RADIUS;
export let MICRO_COLLISION_RADIUS = runtimeConfig.MICRO_COLLISION_RADIUS;
let OBSTACLE_PADDING = runtimeConfig.OBSTACLE_PADDING;

let MICRO_LOW_HEALTH_THRESHOLD = runtimeConfig.MICRO_LOW_HEALTH_THRESHOLD;
let MICRO_RETARGET_COOLDOWN_MS = runtimeConfig.MICRO_RETARGET_COOLDOWN_MS;
let MICRO_CONTACT_INVULNERABILITY_MS = PLAYER_ATTACK_COOLDOWN_MS + 400;
let MICRO_FLEE_DURATION_MS = runtimeConfig.MICRO_FLEE_DURATION_MS;
let MICRO_PATROL_RADIUS = runtimeConfig.MICRO_PATROL_RADIUS;
let MICRO_WAYPOINT_REACH_DISTANCE = runtimeConfig.MICRO_WAYPOINT_REACH_DISTANCE;
let MICRO_WAYPOINT_REFRESH_MS = runtimeConfig.MICRO_WAYPOINT_REFRESH_MS;
let MICRO_ZIG_ANGLE_RADIANS = runtimeConfig.MICRO_ZIG_ANGLE_RADIANS;
let MICRO_ZIG_INTERVAL_MS = runtimeConfig.MICRO_ZIG_INTERVAL_MS;
let MICRO_STEERING_SAMPLE_DISTANCE = runtimeConfig.MICRO_STEERING_SAMPLE_DISTANCE;
const MICRO_STEERING_ANGLES = [Math.PI / 6, Math.PI / 4, Math.PI / 3] as const;

let SMALL_EVOLUTION_SLOT_BASE = runtimeConfig.SMALL_EVOLUTION_SLOT_BASE;
let SMALL_SLOT_LEVEL_DIVISOR = runtimeConfig.SMALL_SLOT_LEVEL_DIVISOR;
let MEDIUM_SLOT_INTERVAL = runtimeConfig.MEDIUM_SLOT_INTERVAL;
let LARGE_SLOT_INTERVAL = runtimeConfig.LARGE_SLOT_INTERVAL;
let MIN_XP_REQUIREMENT = runtimeConfig.MIN_XP_REQUIREMENT;
let BASE_XP_REQUIREMENT = runtimeConfig.BASE_XP_REQUIREMENT;
let XP_REQUIREMENT_GROWTH = runtimeConfig.XP_REQUIREMENT_GROWTH;
let MAX_LEVEL_ITERATIONS = runtimeConfig.MAX_LEVEL_ITERATIONS;

let ORGANIC_MATTER_CELL_SIZE = PLAYER_COLLECT_RADIUS;
...
let CLIENT_TIME_MAX_FUTURE_DRIFT_MS = runtimeConfig.CLIENT_TIME_MAX_FUTURE_DRIFT_MS;

const applyRuntimeConfig = (config: RuntimeConfig): void => {
  runtimeConfig = config;

  MIN_PLAYERS_TO_START = config.MIN_PLAYERS_TO_START;
  WAITING_START_DELAY_MS = config.WAITING_START_DELAY_MS;
  WAITING_START_DELAY_ENABLED = MIN_PLAYERS_TO_START > 1;
  ROUND_DURATION_MS = config.ROUND_DURATION_MS;
  RESET_DELAY_MS = config.RESET_DELAY_MS;
  RECONNECT_WINDOW_MS = config.RECONNECT_WINDOW_MS;
  INACTIVE_TIMEOUT_MS = config.INACTIVE_TIMEOUT_MS;

  MAX_PLAYERS = config.MAX_PLAYERS;
  MAX_CLIENT_MESSAGE_SIZE_BYTES = config.MAX_CLIENT_MESSAGE_SIZE_BYTES;

  MAX_COMBO_MULTIPLIER = config.MAX_COMBO_MULTIPLIER;
  DAMAGE_POPUP_TTL_MS = config.DAMAGE_POPUP_TTL_MS;
  MAX_DAMAGE_POPUPS_PER_TICK = config.MAX_DAMAGE_POPUPS_PER_TICK;

  RATE_LIMIT_WINDOW_MS = config.RATE_LIMIT_WINDOW_MS;
  MAX_MESSAGES_PER_CONNECTION = config.MAX_MESSAGES_PER_CONNECTION;
  MAX_MESSAGES_GLOBAL = config.MAX_MESSAGES_GLOBAL;
  GLOBAL_RATE_LIMIT_HEADROOM = config.GLOBAL_RATE_LIMIT_HEADROOM;
  RATE_LIMIT_UTILIZATION_REPORT_INTERVAL_MS = config.RATE_LIMIT_UTILIZATION_REPORT_INTERVAL_MS;

  WORLD_TICK_INTERVAL_MS = config.WORLD_TICK_INTERVAL_MS;
  SNAPSHOT_FLUSH_INTERVAL_MS = config.SNAPSHOT_FLUSH_INTERVAL_MS;
  PLAYER_ATTACK_COOLDOWN_MS = config.PLAYER_ATTACK_COOLDOWN_MS;
  PLAYER_COLLECT_RADIUS = config.PLAYER_COLLECT_RADIUS;
  CONTACT_BUFFER = config.CONTACT_BUFFER;
  PLAYER_ATTACK_RANGE_BUFFER = CONTACT_BUFFER;
  PLAYER_COLLISION_RADIUS = config.PLAYER_COLLISION_RADIUS;
  MICRO_COLLISION_RADIUS = config.MICRO_COLLISION_RADIUS;
  OBSTACLE_PADDING = config.OBSTACLE_PADDING;

  MICRO_LOW_HEALTH_THRESHOLD = config.MICRO_LOW_HEALTH_THRESHOLD;
  MICRO_RETARGET_COOLDOWN_MS = config.MICRO_RETARGET_COOLDOWN_MS;
  MICRO_CONTACT_INVULNERABILITY_MS = PLAYER_ATTACK_COOLDOWN_MS + 400;
  MICRO_FLEE_DURATION_MS = config.MICRO_FLEE_DURATION_MS;
  MICRO_PATROL_RADIUS = config.MICRO_PATROL_RADIUS;
  MICRO_WAYPOINT_REACH_DISTANCE = config.MICRO_WAYPOINT_REACH_DISTANCE;
  MICRO_WAYPOINT_REFRESH_MS = config.MICRO_WAYPOINT_REFRESH_MS;
  MICRO_ZIG_ANGLE_RADIANS = config.MICRO_ZIG_ANGLE_RADIANS;
  MICRO_ZIG_INTERVAL_MS = config.MICRO_ZIG_INTERVAL_MS;
  MICRO_STEERING_SAMPLE_DISTANCE = config.MICRO_STEERING_SAMPLE_DISTANCE;

  SMALL_EVOLUTION_SLOT_BASE = config.SMALL_EVOLUTION_SLOT_BASE;
  SMALL_SLOT_LEVEL_DIVISOR = config.SMALL_SLOT_LEVEL_DIVISOR;
  MEDIUM_SLOT_INTERVAL = config.MEDIUM_SLOT_INTERVAL;
  LARGE_SLOT_INTERVAL = config.LARGE_SLOT_INTERVAL;
  MIN_XP_REQUIREMENT = config.MIN_XP_REQUIREMENT;
  BASE_XP_REQUIREMENT = config.BASE_XP_REQUIREMENT;
  XP_REQUIREMENT_GROWTH = config.XP_REQUIREMENT_GROWTH;
  MAX_LEVEL_ITERATIONS = config.MAX_LEVEL_ITERATIONS;

  ORGANIC_MATTER_CELL_SIZE = PLAYER_COLLECT_RADIUS;

  CLIENT_TIME_MAX_FUTURE_DRIFT_MS = config.CLIENT_TIME_MAX_FUTURE_DRIFT_MS;
};
